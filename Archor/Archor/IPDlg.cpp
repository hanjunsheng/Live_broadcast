...
void CIPDlg::OnBnClickedButton1()
{
	...
	((CArchorDlg *)GetParent())->GetIMediator()->SetServerAddr(address);

	//连接tcp
	if(false == ((CABsIMediator *)((CArchorDlg *)GetParent())->GetIMediator())->m_tcp->Connect())
	{
		return ;
	}
	...
	//更改对话框
	ShowWindow(SW_HIDE);
	((CArchorDlg *)AfxGetMainWnd())->m_clogindlg.ShowWindow(SW_SHOW);
	((CArchorDlg *)AfxGetMainWnd())->m_clogindlg.InitialDlg();
	((CArchorDlg *)AfxGetMainWnd())->SetMainWnd(((CArchorDlg *)AfxGetMainWnd())->m_clogindlg);
	((CArchorDlg *)AfxGetMainWnd())->m_clogindlg.SetFocus();
}
