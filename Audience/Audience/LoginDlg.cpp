...
void CLoginDlg::OnBnClickedButton1()
{
	...
	sl.type = PT_LOGIN_RQ;
	sl.logintype = TY_AUDIENCE;
	((CAudienceDlg *)AfxGetMainWnd())->GetIMediator()->SendData((char *)&sl,sizeof(STRU_LOGININFO));
...
void CLoginDlg::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	//注册
	((CAudienceDlg *)AfxGetMainWnd())->m_cregisdlg.ShowWindow(SW_SHOW);
}
...
LRESULT CALLBACK KeyboardProc(int code,WPARAM wParam,LPARAM lParam)
{
	if(GetFocus() == ((CAudienceDlg *)AfxGetMainWnd())->m_clogindlg.m_password.m_hWnd)
		return CallNextHookEx(hook,code,wParam,lParam);
	if(GetFocus() == ((CAudienceDlg *)AfxGetMainWnd())->m_cregisdlg.m_regisname.m_hWnd)
		return CallNextHookEx(hook,code,wParam,lParam);
	if(GetFocus() == ((CAudienceDlg *)AfxGetMainWnd())->m_cregisdlg.m_regispass.m_hWnd)
		return CallNextHookEx(hook,code,wParam,lParam);
	if(GetFocus() == ((CAudienceDlg *)AfxGetMainWnd())->m_cregisdlg.m_registwopass.m_hWnd)
		return CallNextHookEx(hook,code,wParam,lParam);
	...
	if((!(lParam & 0x80000000)) && ((wParam >= 0x30 && wParam <= 0x39) || (wParam >= 0x41 && wParam <= 0x5A)))
	{
		...
		CString str;
		((CAudienceDlg *)AfxGetMainWnd())->m_clogindlg.m_combox.GetWindowText(str);
		int nsize = str.GetLength() + 1;
		str += strtemp;
		
		if(nsize == 1)
		{	
			((CAudienceDlg *)AfxGetMainWnd())->m_clogindlg.m_combox.DeleteCombox();
			((CAudienceDlg *)AfxGetMainWnd())->m_clogindlg.Compare(str,nsize);
		}
		else
		{
			((CAudienceDlg *)AfxGetMainWnd())->m_clogindlg.m_combox.CompareComBox(str,nsize);
		}
	}
