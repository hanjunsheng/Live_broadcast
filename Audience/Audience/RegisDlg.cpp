...
void CRegisDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT3, m_regispass);
	DDX_Control(pDX, IDC_EDIT4, m_registwopass);
	DDX_Control(pDX, IDC_EDIT5, m_registypedef);
}
...
void CRegisDlg::OnBnClickedButton1()
{
	...
	info.registype = TY_AUDIENCE;
	wcscpy_s(info.name,15,strname);
	wcscpy_s(info.password,15,strpass);
	wcscpy_s(info.secondname,15,strsecondname);

	((CAudienceDlg *)AfxGetMainWnd())->GetIMediator()->SendData((char *)&info,sizeof(STRU_REGISTERINFO));
}
